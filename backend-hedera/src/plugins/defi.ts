...
class GetTokenPriceTool extends StructuredTool {
  ...
  async _call(input: z.infer<typeof this.schema>): Promise<string> {
    try {
      ...
      return `Current price of token ${input.tokenId}: $${price.toFixed(
        4
      )} USD`;
    } catch (error) {
...
class SwapTokensTool extends StructuredTool {
  ...
  schema = z.object({
    fromTokenId: z
      .string()
      .describe("The ID of the token to swap from (e.g., 0.0.12345)"),
    toTokenId: z
      .string()
      .describe("The ID of the token to swap to (e.g., 0.0.67890)"),
    amount: z
      .number()
      .positive()
  ...
  async _call(input: z.infer<typeof this.schema>): Promise<string> {
    try {
      ...
      return `Simulated swap of ${input.amount} tokens (${
        input.fromTokenId
      }) for ${receivedAmount.toFixed(4)} tokens (${
        input.toTokenId
      }).\n\nNote: This is a mock implementation. In a real implementation, this would execute the swap through a DEX on Hedera.`;
    } catch (error) {
      return `Error performing token swap: ${
        error instanceof Error ? error.message : String(error)
      }`;
    }
  }
...
class CheckTokenBalanceTool extends StructuredTool {
  ...
  schema = z.object({
    ...
    accountId: z
      .string()
      .optional()
      .describe("The account ID to check (defaults to the operator account)"),
  });
  ...
  async _call(input: z.infer<typeof this.schema>): Promise<string> {
    try {
      ...
      return `Error checking token balance: ${
        error instanceof Error ? error.message : String(error)
      }`;
    }
  }
